...
public class OptionalLocalDateOf implements Supplier<Optional<LocalDate>> {
    ...
    public OptionalLocalDateOf(final JSONObject origin, final String name) {
        this(new OptionalOf<>(origin, name, (json, field) -> new LocalDateOf(json.getString(field)).get()));
    }

    public OptionalLocalDateOf(final Supplier<Optional<LocalDate>> origin) {
        this.origin = origin;
    }
